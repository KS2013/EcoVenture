...
const FriendsModule = {
    ...
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        ...
        if (user && user.id !== auth.authUser?.id) {
          ...
          window.EcoVentureUI.showToast("That's your own code!", 'warning');
          searchResult.classList.add('hidden');
          this.foundFriend = null;
        } else {
          searchResult.classList.add('hidden');
  ...
  async sendRequest() {
    if (!this.foundFriend) {
      window.EcoVentureUI.showToast('Enter a valid friend code first', 'warning');
      return;
    }
    ...
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        await window.EcoVentureAuth.sendFriendRequest(auth.authUser.id, this.foundFriend.id);
        window.EcoVentureUI.showToast('Friend request sent!', 'success');
        this.closeAddModal();
      } catch (error) {
        window.EcoVentureUI.showToast(error.message || 'Failed to send request', 'error');
      }
    } else {
      window.EcoVentureUI.showToast('Friend request sent! (Demo)', 'success');
      this.closeAddModal();
    }
  },
  ...
  copyFriendCode() {
    ...
    navigator.clipboard.writeText(code).then(() => {
      window.EcoVentureUI.showToast('Friend code copied!', 'success');
    }).catch(() => {
      window.EcoVentureUI.showToast('Failed to copy', 'error');
    });
  },
  ...
  async loadRequests() {
    ...
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        const received = await window.EcoVentureAuth.getPendingFriendRequests(auth.authUser.id);
        this.renderReceivedRequests(received, receivedList);

        const sent = await window.EcoVentureAuth.getSentFriendRequests(auth.authUser.id);
      ...
      return `
        <div class="request-item" data-id="${req.id}">
          ...
          <div class="request-actions">
            <button class="btn btn-accept" onclick="window.EcoVentureFriends.acceptRequest('${req.id}')">Accept</button>
            <button class="btn btn-reject" onclick="window.EcoVentureFriends.rejectRequest('${req.id}')">Reject</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  async acceptRequest(requestId) {
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        ...
        window.EcoVentureUI.showToast('Friend added!', 'success');
        this.loadRequests();
        this.updatePendingBadge();
        this.loadLeaderboard();
      } catch (error) {
        window.EcoVentureUI.showToast('Failed to accept request', 'error');
      }
    }
  },
  ...
  async rejectRequest(requestId) {
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        ...
        window.EcoVentureUI.showToast('Request declined', 'success');
        this.loadRequests();
        this.updatePendingBadge();
      } catch (error) {
        window.EcoVentureUI.showToast('Failed to decline request', 'error');
      }
    }
  },
  ...
  async loadLeaderboard() {
    ...
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        leaderboardData = await window.EcoVentureAuth.getFriendsLeaderboard(auth.authUser.id);
        this.updatePendingBadge();
      } catch (error) {
    ...
    if (leaderboardData.length <= 1) {
      listEl.innerHTML = `
        <div class="leaderboard-empty">
          ...
          <button class="btn btn-primary" onclick="window.EcoVentureFriends.openAddModal()">Add Your First Friend</button>
        </div>
      `;
    ...
    window.EcoVentureLeaderboard.renderLeaderboard(listEl, leaderboardData, yourRankCard);
  ...
  async updatePendingBadge() {
    ...
    if (window.EcoVentureAuth && window.EcoVentureAuth.isConfigured()) {
      try {
        const count = await window.EcoVentureAuth.getPendingRequestCount(auth.authUser.id);
        if (count > 0) {
...
// Export
window.EcoVentureFriends = FriendsModule;
